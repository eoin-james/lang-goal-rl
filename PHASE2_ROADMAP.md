...
|---|-------|-------|-----------|------------|--------|--------|
| 7 | Frame-of-reference check | `AXIS_DIRECTIONS` (`goal_region_vocabulary.py`), `episode_recording.record_episode`'s render path | Before/after frame pairs per direction | Human visual sign-off that each direction looks correct on camera (deliberately not a numeric gate — see report) | Awaiting human sign-off | — |
