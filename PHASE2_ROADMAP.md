...
| 9 | Waypoint following | Same checkpoint, `rollout_with_goal_switch` | `waypoint_following.py` — `rollout_with_waypoints` | N=2 reduces exactly to stage 5's `rollout_with_goal_switch` result (regression test); N=3-5 chains don't show compounding degradation | **INCONCLUSIVE (1 seed) — N=2 equivalence fully settled (inherits stage 5's 10-seed validation). N=3-5 chains show no compounding on seed_0 alone (7 isolated single-leg failures across 600 episodes, zero multi-leg), but a single checkpoint can't rule out checkpoint-dependent compounding — sent back for an 8-healthy-seed re-run before Done.** | [report](experiments/09_waypoint_following/report.md) |
